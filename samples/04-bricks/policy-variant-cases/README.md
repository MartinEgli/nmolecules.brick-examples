...
| `attribute-multi-policy-cases` | Class-scoped `[Policy]`, `[PolicyImport]`, `[Rule]`, `[Dependency]`; plus `AttributeMultiPolicyCases.Assembly.cs` with `[assembly: Policy]`, `[assembly: Rule]`, `[assembly: Dependency]` | Type and namespace dependencies across platform/product/team policies | Multiple attribute policies can be composed without extra rule/dependency policy IDs when each policy has a dedicated definition class. The assembly-file variant shows the convention trade-off when several policies share one assembly attribute scope. |
...
It also includes `AttributeMultiPolicyCases.Assembly.cs` because the Bricks
attributes support assembly scope. In that variant several policies live in one
assembly file. Since `RuleAttribute` and `DependencyAttribute` do not currently
carry an owning policy ID, the sample groups them by ID prefix and tests that
this convention is visible as a trade-off.
