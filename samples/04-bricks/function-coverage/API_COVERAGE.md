...
|---|---|---|
| Attribute markers: `RoleAttribute`, `RoleAliasAttribute`, `RuleAttribute`, `RuleFilterAttribute`, member-cardinality attributes | `domain-language-kits/`, `member-contracts/`, `rule-filters/`, `violations/` | Consumers mark roles, aliases, rules, filters and member contracts directly in code. |
| DDD role modeling with Bricks | `ddd-building/` | Consumers can build aggregate, entity, value object, repository, factory, domain service and application service roles as a focused DDD slice, either through attributes only or through explicit policy objects. |
