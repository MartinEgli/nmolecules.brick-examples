...
| Policy definition variants: attribute-only, attribute multi-policy, code policy, JSON policy, composed multi-policy | `policy-variant-cases/` | Teams can choose a policy authoring style, combine platform/product/team policies, and see when attribute-only correlation needs conventions, definition classes, or future explicit IDs. |
